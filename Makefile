...
help:
	@echo "C++ Live Transcription - Available Commands"
	@echo "============================================"
	@echo ""
	@echo "Setup:"
	...
	@echo "  make install           Install backend dependencies only (build)"
...
init: check-prereqs
	...
	cmake -B build -DCMAKE_TOOLCHAIN_FILE=$$(vcpkg --vcpkg-root 2>/dev/null || echo vcpkg)/scripts/buildsystems/vcpkg.cmake 2>/dev/null || cmake -B build
	cmake --build build
	@echo ""
...
# Install backend dependencies (build the project)
install:
	@echo "==> Building backend..."
	cmake -B build -DCMAKE_TOOLCHAIN_FILE=$$(vcpkg --vcpkg-root 2>/dev/null || echo vcpkg)/scripts/buildsystems/vcpkg.cmake 2>/dev/null || cmake -B build
	cmake --build build
